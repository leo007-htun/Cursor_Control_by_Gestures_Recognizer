...
``mediapipe`` the MediaPipe Hands module from the MediaPipe library to detect and track hand landmarks in real-time from webcam feed. The specific model used is a pre-trained model provided by MediaPipe for hand tracking.
    ...
    model_complexity=0: This parameter controls the complexity of the hand tracking model. A value of 0 indicates the simplest model.
    min_detection_confidence=0.5: This parameter sets the minimum confidence threshold for a hand detection to be considered valid.
    min_tracking_confidence=0.5: This parameter sets the minimum confidence threshold for hand landmarks to be considered during tracking.
